...
export function SceneGrid({
  job, onSelectVersion, onRegenerateScene, onApproveScene,
  onGenerateNext, onUnlock, onSaveClipEdits, onInspect,
}: Props) {
  // ── Sistema DeepL (copia exacta del monolito) ──
  ...
  // ── Helper: llamada al proxy DeepL (1:1 del monolito) ──
  ...
  // ── Toggle Traducción Prompt (1:1 del monolito) ──
    ...
    try {
      if (!isCurrentlyES) {
        ...
        if (toTranslate.length > 0) {
          ...
          const textsToTranslate = toTranslate.map((t, i) => {
            if (t.compositeKey.toUpperCase().includes("DIALOGUE")) {
              const match = t.text.match(/(['"])([^'"]*)(["'])/);
              if (match) { protectedMap[i] = match[2]; return t.text.replace(match[2], "PROTECTED_SPOKEN_TEXT"); }
            }
            return t.text;
          });
        ...
        if (dirtyList.length > 0) {
          const protectedMap: Record<number, string> = {};
          const textsToTranslate = dirtyList.map((tag, i) => {
            const compositeKey = `${sceneKey}___${tag}`;
            const text = liveValuesRef.current[compositeKey] || esCache[compositeKey] || "";
            if (tag.toUpperCase().includes("DIALOGUE")) {
              const match = text.match(/(['"])([^'"]*)(["'])/);
              if (match) { protectedMap[i] = match[2]; return text.replace(match[2], "PROTECTED_SPOKEN_TEXT"); }
            }
            return text;
          });
  ...
  // ── Toggle Traducción Script (1:1 del monolito) ──

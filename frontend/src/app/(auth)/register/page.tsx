...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // TODO: Conectar con FastAPI en Fase 9 para registrar en BDD
      // fetch('/api/register', { method: 'POST', ... })
      
      // Simulamos registro y login automático para la migración
  ...
  const handleGoogleLogin = async () => {
    ...
    await signIn("google", { callbackUrl: "/estudio" });
  };
    ...
    <Card className="border-white/10 bg-black/40 backdrop-blur-xl shadow-2xl">
      <CardHeader className="space-y-1 text-center">
        ...
        <CardDescription className="text-zinc-400">
          Únete a UGC Automator y empieza a generar
        </CardDescription>
      </CardHeader>
        ...
        <CardFooter className="flex flex-col gap-4">
          ...
          <div className="relative w-full">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-[#0a0a0f] px-2 text-zinc-500">O regístrate con</span>
            </div>
          </div>
